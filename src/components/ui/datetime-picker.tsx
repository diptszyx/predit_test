    ...
    <Popover open={open} onOpenChange={setOpen} modal={modal}>
      <PopoverTrigger className="w-full">
        {renderTrigger ? (
          ...
          <div
            className={cn(
              'flex w-full cursor-pointer items-center h-9 ps-3 pe-1 font-normal border border-input rounded-md text-sm',
      ...
      <PopoverContent className="w-auto p-2 bg-background" side="bottom">
...
function TimePicker({
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger>
        <Button variant="outline" role="combobox" aria-expanded={open} className="mt-0.5 w-full justify-between">
          <Clock className="mr-2 size-4" />
          {display}
          <ChevronDownIcon className="ml-2 size-4 shrink-0 opacity-50" />
        </Button>
      </PopoverTrigger>
      <PopoverContent className="p-0 bg-background" side="bottom">
