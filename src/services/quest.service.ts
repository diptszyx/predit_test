...
export enum QuestType {
  ...
  DAILY_TRADE_PREDIT_MARKET = 'DAILY_TRADE_PREDIT_MARKET',
}
...
export const DAILY_QUEST_TYPES: QuestType[] = [
  ...
  QuestType.DAILY_TRADE_PREDIT_MARKET,
];
...
export type Quest =
  ...
  | (BaseQuest & {
      questType: QuestType.DAILY_TRADE_PREDIT_MARKET;
      metadata: null;
    });
...
export const verifyTradeDailyPreditMarket = async (questId: string) => {
  const response = await apiClient.post<VerifyQuestResponse>(
    '/quests/verify-daily-trade-predit-market',
    {
      questId,
    },
  );
