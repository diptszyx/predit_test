...
const SellingModal = ({
  ...
  const { avgUSD } = calculateAvgAndPnL({
  ...
  const handleSelling = async () => {
    ...
    try {
      let result = await redeemPositions(position.mint, shares, market.ticker);
    ...
    >
      ...
      <DialogContent className="w-full">
        <div className="flex flex-col items-center justify-center gap-2 px-2">
          ...
          <p className="w-2/3 text-center text-xs">{position.market.title}</p>
          <div
            className="
                mt-3 w-full
                rounded-2xl
              ...
              "
          >
            <div className="text-center text-base font-semibold">Receive</div>
            ...
            <div className="mt-3 text-center text-sm">
              Selling <span className="">{shares.toFixed(2)}</span> shares @{' '}
              <span className="">
