...
import { TrendingUp, Trophy, Flame, Star, Target, Zap, ArrowRight, Users, Award, Lock, Info } from "lucide-react";
import { mockUser, dailyQuests, platformStats, houses } from "../lib/mockData";
import { getLevelTitle, getXPForNextLevel, getXPForCurrentLevel, getLevelProgress } from "../lib/xpSystem";
...
export function Dashboard({ onNavigate, onBackToOracles, totalOracles = 22, user = mockUser }: DashboardProps) {
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <h3 className="mt-1">{(user.totalPredictions || 42).toLocaleString()}</h3>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <h3 className="mt-1">{user.oraclesVisited}/{totalOracles}</h3>
              </div>
        ...
        <Card className="border-purple-500/30 bg-gradient-to-r from-purple-500/10 to-pink-500/10">
          <CardContent className="p-4">
            <div className="flex items-center gap-4">
              <div className={`w-14 h-14 rounded-xl bg-gradient-to-br ${userHouse.color} flex items-center justify-center text-2xl flex-shrink-0`}>
                {userHouse.icon}
              </div>
              <div className="flex-1">
                ...
                <p className="text-sm text-muted-foreground">
                  Season 1 - The Summons ends in {daysUntilSeasonEnd} days (Nov 30). 
                  {!isSwitchingLocked && remainingSwitches > 0 && ` ${remainingSwitches} house ${remainingSwitches === 1 ? 'switch' : 'switches'} left (locks Nov 20).`}
                  {isSwitchingLocked && ' House switching locked until season end.'}
                </p>
              </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-muted-foreground mb-2">
                  Choose from 20 unique houses. Earn +1.1x XP multiplier and compete for massive season end rewards!
                </p>
      ...
      <Card className="border-border">
        <CardContent className="p-6">
          <div className="flex items-center justify-between mb-2">
            <div>
              <h3 className="mb-1">Level {user.level}: {levelTitle}</h3>
              <p className="text-sm text-muted-foreground">
        ...
        <Card className="border-border">
          <CardContent className="p-6">
            <div className="text-center">
              ...
              <div className="text-2xl mb-1">{platformStats.totalUsers.toLocaleString()}</div>
        ...
        <Card className="border-border">
          <CardContent className="p-6">
            <div className="text-center">
              ...
              <div className="text-2xl mb-1">{(platformStats.totalPredictions / 1000).toFixed(0)}K+</div>
        ...
        <CardContent className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-sm text-muted-foreground">House Switching</span>
                  <Badge variant="outline" className={isSwitchingLocked ? "bg-red-500/10 border-red-500/30" : "bg-green-500/10 border-green-500/30"}>
                    {isSwitchingLocked ? <><Lock className="w-3 h-3 mr-1" />Locked</> : `${remainingSwitches} left`}
                  </Badge>
                </div>
              </div>
            </div>
          ...
          {user.house && userHouse && (
            <div className="mt-4 p-3 rounded-lg bg-purple-500/10 border border-purple-500/30">
              <p className="text-sm">
                <strong>Your Projected Bonus:</strong> If {userHouse.name} maintains rank #{userHouse.rank}, 
                you'll receive a <strong>{getSeasonMultiplier(userHouse.rank)}</strong> multiplier on all Season 1 XP at season end!
              </p>
            </div>
          )}
        </CardContent>
